...
- [x] `GET /api/illness/:id/report?format=pdf|text` — generates a clean, readable document of the episode
- [x] Report contents:
...
- [x] PDF output: clean, 1–3 pages, readable aloud during a phone call with a nurse
- [x] Text/plain output: simple line-by-line format for copy-paste into a patient portal or message
- [x] Client: "Share Report" button on the illness detail page → triggers download; iOS share sheet opens on mobile
...
#### Data Export Page (`/settings/export`) ✅ Complete
...
**Raw Data Export:**
- [x] **Data type selector** — toggle rows: Feedings, Sleep, Diapers, Weight & Height, Medications, Tummy Time, Mood & Activity (default: all)
- [x] **Date range picker** — quick chips (Today, 7 days, 30 days, All time) + custom from/to date inputs
- [x] **Format selector** — PDF (formatted, readable) and CSV (raw, for spreadsheets)
- [x] **Download button** — triggers server-side generation; shows a loading state; downloads file (filename from Content-Disposition)
- [x] Server: `GET /api/export?babyId=&types=&from=&to=&format=` — sends PDF, single CSV, or ZIP of CSVs (dependency-free ZIP writer in `lib/zip.ts`)
- [x] `GET /api/export/preview` — record counts per type so the page can show "7-day PDF — 142 records" before downloading
- [x] PDF format: one section per data type, chronological tables via shared `PdfReportBuilder`
- [x] CSV format: one file per data type, standard columns, UTC timestamps

**Health Summary Report:**
- [x] Separate "Health Summary" tab on the export page
- [x] Customizable sections (toggles): Vaccinations Received, Current Medications, Recent Weight & Growth, Feeding Overview (last 30 days), Sleep Overview (last 30 days)
- [x] Short PDF output — clean, readable, suitable for handing to a pediatrician or daycare
- [x] Baby info header: name (if set), date of birth, report generated date
- [x] Prominent disclaimer on every page: *"This is an informal record generated by a private family app — not an official medical document."* (PdfReportBuilder footer)
- [x] Server: `GET /api/export/health-summary?babyId=&sections=` — generates PDF
- [x] No date range needed for vaccinations/medications; feeding/sleep use last 30 days by default; empty sections omitted
...
- [x] Web Push notifications via VAPID (shipped earlier with SOS/wake-window work)
- [x] Notification types: feeding reminder (configurable interval via Settings; cron checks every 5 min with per-interval cooldown), wake window alert (existing, now toggleable)
...
- Offline logs sync correctly when connection restores — deferred with write queue

#### Settings Page ✅ Complete (one follow-up)
